...
class ShoppinglistListView:
    ...
    def _initialize_items_none(self):
        item_frame = ttk.Frame(master=self._frame)
        label_none = ttk.Label(
            master=item_frame,
            text='Ostoslista on tyhjä',
            foreground='orange')
...
class MessagelistListView:
    ...
    def _initialize_message_message(self, message):
        ...
        label_user = ttk.Label(
            ...
            text=(f'Kirjoittaja: {message.user}'))
...
class ItemsView:
    ...
    def _initialize_delete_all_items(self):
        shopping_list_label = ttk.Label(
            master=self._frame,
            text='TÄMÄN HETKINEN OSTOSLISTASI:')
        delete_all_items_button = ttk.Button(
            master=self._frame,
            text='Poista kaikki tuotteet',
            command=self._handle_delete_all_items)
    ...
    def _initialize_messages_header(self):
        messages_header_label = ttk.Label(
            master=self._frame,
            text='TÄMÄN HETKISET VIESTISI:')
    ...
    def _initialize_message_field(self):
        ...
        self._create_message_entry.grid(
            row=4,
            column=0,
            columnspan=3,
            padx=5,
